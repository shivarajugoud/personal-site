...
export default async function handler(req) {
  ...
  const fullMessages = buildMessages(PERSONAL_CONTEXT, summary, chatMessages)
  ...
  for (const model of chain) {
    try {
      ...
      ;(async()=>{
        await writer.write(enc.encode(`data: ${JSON.stringify({model:label,compressed:summary!==null})}\n\n`))
